...
public class JDBCProfileRepository extends BaseRepository implements ProfileRepository {
    ...
    private final RowMapper<Profile> profileMapper = rs -> Profile.builder()
    ...
    public PagedResponse<Profile> findAll(int page, int size) {
        ...
        String sql = "SELECT id, name, COUNT(*) OVER() as total_count FROM profiles ORDER BY name LIMIT ? OFFSET ?";
    ...
    public Profile findById(UUID profileId) {
        List<Profile> results = query("SELECT id, name FROM profiles WHERE id = ?", profileMapper, profileId);
